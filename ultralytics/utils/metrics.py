...
def bbox_iou(
    ...
    if xywh:  # transform from xywh to xyxy
        # 把输入的 中心点坐标 + 宽高 转换成 左上右下 格式的 box
        # chunk()函数把输入的 4 维张量按维度切分，返回一个元组，元组中每个元素都是一个张量
    ...
    # Intersection area 计算两个框的交集面积。
    # clamp_(0) 就是把所有小于 0 的值截断成 0，保证宽度非负。
    ...
    if CIoU or DIoU or GIoU or EIoU:
        # 计算包围盒宽度和高度（包围盒就是直接把box1和box2包起来的最小值）
