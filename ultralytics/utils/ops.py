...
def NMS(boxes, scores, iou_thres, class_nms='CIoU'):
    ...
    if class_nms == 'CIoU':
        ...
        SIoU=True
    # 返回一个 排序后的索引（indices），而不是直接对原张量排序后的值。
    ...
    while B.numel() > 0:
        ...
        if B.numel() == 1: break
        # bbox_iou()支持多个框进行运算
        iou = bbox_iou(boxes[index, :], boxes[B[1:], :], GIoU=GIoU, DIoU=DIoU, CIoU=CIoU, EIoU=EIoU)
...
# 1️⃣ 置信度筛选（先把低分框丢掉）
# 2️⃣ 类别筛选（只要指定类别）
# 3️⃣ 坐标格式转换（xywh → xyxy）
# 4️⃣ 是否要多标签（multi-label）
# 5️⃣ 非极大值抑制（NMS）
# 6️⃣ 处理 batch，每张图独立做 NMS
# 7️⃣ 支持 Oriented Bounding Box、Mask、End2End 等特性
