...
from ultralytics.nn.block.HTB import HTB
...
#         g: int = 1,
#         shortcut: bool = True,
#         use_htb: bool = False,  # ←← 新增：必须放在最后，保持兼容
#     ):
#         """
#         Initialize C3k2 module.
#
...
#             g (int): Groups for convolutions.
#             shortcut (bool): Whether to use shortcut connections.
#             use_htb (bool): If True, replace the last block with HTB (Histogram Transformer Block).
#         """
#         super().__init__(c1, c2, n, shortcut, g, e)  # 确保传递正确的参数顺序
#         self.use_htb = use_htb
#
#         # Build the module list
#         self.m = nn.ModuleList()
#         for i in range(n):
#             if i == n - 1 and use_htb:
#                 # 替换最后一个 block 为 HTB
#                 self.m.append(HTB(c1=self.c, num_heads=4, ffn_expansion_factor=2.5))
#             else:
#                 # 使用 C3k 或 Bottleneck
...
#
#     def forward(self, x):
#         """Forward pass with optional HTB in the last block."""
#         if self.use_htb and len(self.m) > 0:
#             y = x
#             # 前 n-1 个 block 正常残差叠加
#             for m in self.m[:-1]:
#                 y = y + m(x)
#             # 最后一个 block 用 HTB
#             if self.m:
...
# class C3k2_HTB(nn.Module):
#     """C3k2 with optional HTB at the last block. Compatible with C3k2's YAML usage."""
#
#     def __init__(
#             self,
#             c1: int,
#             c2: int,
#             n: int = 2,  # 默认 2，匹配 C3k2 默认行为
#             c3k: bool = False,  # 是否用 C3k
#             shortcut: bool = True,
#             use_htb: bool = False  # ← 新参数，放最后，保持兼容
#     ):
#         """
#         Args:
#             c1 (int): 输入通道
#             c2 (int): 输出通道
#             n (int): 模块数量
#             c3k (bool): 是否使用 C3k 结构
#             shortcut (bool): 是否使用残差
#             use_htb (bool): 是否将最后一个 block 替换为 HTB
#         """
#         super().__init__()
#         self.c2 = c2
#         self.n = n
...
#             else:
#                 self.m.append(
#                     C3k(self.c, self.c, 2, shortcut, g=1) if c3k else
#                     Bottleneck(self.c, self.c, shortcut, g=1)
#                 )
#
#     def forward(self, x):
#         y = list(self.cv1(x).chunk(2, 1))  # split into 2
#         for m in self.m:
#             y.append(m(y[-1]))
#         return self.cv2(torch.cat(y, 1))
#
...
#         return self.cv2(torch.cat(y, 1))
class C3k2_HTB(nn.Module):
    def __init__(self, c1, c2, n=2, c3k=False, shortcut=True, use_htb=False):
        super().__init__()
        self.c = int(c2 * 0.5)  # e=0.5
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
        self.cv2 = Conv((2 + n) * self.c, c2, 1)
    ...
    def forward_split(self, x):
        y = [v for v in self.cv1(x).split((self.c, self.c), 1)]
        for m in self.m:
            y.append(m(y[-1]))
        return self.cv2(torch.cat(y, 1))
...
# 改进三：对应yaml文件是 yolo11-C3CA.yaml
# 在C3模块上添加CA注意力机制，然后替换C3K2模块为C3CA模块。
...
# 改进四：对应yaml文件是 yolo11-C3ECA.yaml
# 在C3模块上添加ECA注意力机制，然后替换C3K2模块为C3ECA模块。
class ECABottleneck(nn.Module):
    ...
    def forward(self, x):
        ...
        y = self.sigmoid(y)
        out = x1 * y.expand_as(x1)  # 对应通道乘以系数
...
# 改进五：更换backbone为MobileNetV3
# 对应的yaml文件为：yolo11-MobileNetV3.yaml
class h_sigmoid(nn.Module):
    def __init__(self, inplace=True):
        super(h_sigmoid, self).__init__()
        self.relu = nn.ReLU6(inplace=inplace)
...
# yolo模型改进七：更换backbone为efficientnetV2
...
# class C3k2_HSB(C2f):
#     """
#     C3k2 module with optional HTB (Histogram Transformer Block) insertion.
#     """
#     def __init__(
#         self,
#         c1: int,
...
#             c2: 输出通道
#             n: 模块数量
#             c3k: 是否使用 C3k 替代 Bottleneck
#             e: 扩展系数
#             g: 分组卷积
#             shortcut: 是否使用残差
#             use_htb: 是否使用 HTB（HSB）
#             htb_idx: 插入 HTB 的位置（从0开始，-1表示最后一个）
#         """
#         super().__init__(c1, c2, n, shortcut, g, e)
#         blocks = []
#         for i in range(n):
#             if use_htb and i == (htb_idx if htb_idx >= 0 else n + htb_idx):
#                 blocks.append(HTB(c1=self.c))
#             else:
#                 if c3k:
