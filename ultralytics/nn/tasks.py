...
from ultralytics.nn.Conv.SPD_Conv import space_to_depth, SPDConv
from ultralytics.nn.neck.GFPN import CSPStage
from ultralytics.nn.attention.LargeSeparableKernelAttention import SPPF_LSKA
from ultralytics.nn.attention.attention import *
from ultralytics.nn.block.CMUNeXt import C3k2_CMUNeXtBlock,CMUNeXtBlock
from ultralytics.nn.updownsample.dysample import DySample
from ultralytics.nn.backbone.VanillaNet import VanillaBlock
from ultralytics.nn.modules.block import C3k2_HTB
from ultralytics.nn.Conv.LargeSeparableKernelAttention import C3k2_LSKA
from ultralytics.nn.Conv.dynamic_snake_conv import C3k2_DySnakeConv
from ultralytics.nn.Conv.deformable_LKA import C3k2_deformable_LKA
from ultralytics.nn.attention.EMA import EMA_attention
from ultralytics.nn.backbone.mobilenetv4block import C3k2_UIB
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            {Detect, WorldDetect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB, ImagePoolingAttn, v10Detect, Detect_ASFF}
        ):
            args.append([ch[x] for x in f])
            if m is Segment or m is YOLOESegment:
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)
            if m in {Detect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB, Detect_ASFF}:
                m.legacy = legacy
        elif m is CBAM:
            c1, c2 = ch[f], args[0]
            if c2 != nc:
            ...
            c1 = ch[f]
            c2 = args[1] if len(args) > 1 else c1  # SPDConv(c1, c2)，c2 是 args[1]
        elif m is DySample:
            c2 = ch[f]
            args = [c2, *args]
